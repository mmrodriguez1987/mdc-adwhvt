...
namespace UnitTest.Model.DataWarehouse
{
    public class Address
    {
        ...
        public Address(string cnnDWH, string ccnCCB, string ccnValTest)
        {
            _ccnDWH = cnnDWH;
            _ccnCCB = ccnCCB;
        ...
        public Task<DataSet> AddressCount(DateTime startDate, DateTime endDate)
        {
            ...
            return Task.Run(() =>
            {
                try
                {
                    queryDWH = "SELECT COUNT(SRC_ADDR_ID) DTW_Count FROM dwadm2.CD_ADDR WHERE DATA_LOAD_DTTM BETWEEN @startDate AND @endDate";
                    ...
                    testInterpretation = results.createMessageNotification(TestResult.BusinessStar.BU, TestResult.Entity.DimAddress, TestResult.TestGenericName.Distinct, ccbCount, dwhCount, appEnv, !stateTest);
                ...
                {
                    DataRow dr = myResponse.Tables[0].NewRow();
                    dr["description"] = ("Error Reading ADDR Count from CCB: " + e.ToString().Substring(0, 198));
                    myResponse.Tables[0].Rows.Add(dr);
                    return myResponse;
        ...
        /// <param name="endDate">End Evaluated Date</param>
        /// <returns>a Dataset with a structure ready to be converted in JSON with Details about the test result</returns>
        public Task<DataSet> NewAddressCount(DateTime startDate, DateTime endDate)
        {
            ...
            return Task.Run(() =>
            {
                try
                {
                    ...
                    testInterpretation = results.createMessageNotification(TestResult.BusinessStar.BU, TestResult.Entity.DimAddress, TestResult.TestGenericName.New, ccbCount, dwhCount, appEnv, !stateTest);
                ...
                {
                    DataRow dr = myResponse.Tables[0].NewRow();
                    dr["description"] = ("Error Reading New ADDR Count from CCB: " + e.ToString().Substring(0, 198));
                    myResponse.Tables[0].Rows.Add(dr);                                
                    return myResponse;
                }
            });
        ...
        /// <param name="endDate">End Evaluated Date</param>
        /// <returns>a Dataset with a structure ready to be converted in JSON with Details about the test result </returns>
        public Task<DataSet> UpdatedAddressCount(DateTime startDate, DateTime endDate)
        {
            ...
            return Task.Run(() =>
            {
                try
                {
                    ...
                    var UpdatedADDRisesOnCDC = from row in evalDataCCB.Tables[0].AsEnumerable()
                                               where row.Field<Int32>("toInsert") == 0 && row.Field<Int32>("__$operation") == 4
                                               select row;

                    ccbCount = UpdatedADDRisesOnCDC.Count();
                    dwhCount = evalDataDWH.Tables[0].Rows.Count;

                    bool stateTest = (ccbCount >= 0 && dwhCount >= 0 && (ccbCount == dwhCount));

                    testInterpretation = results.createMessageNotification(TestResult.BusinessStar.BU, TestResult.Entity.DimAddress, TestResult.TestGenericName.Updated, ccbCount, dwhCount, appEnv, !stateTest);
                ...
                {                   
                    DataRow dr = myResponse.Tables[0].NewRow();
                    dr["description"] = ("Error Reading Updated ADDR Count from CCB: " + e.ToString().Substring(0, 198));
                    myResponse.Tables[0].Rows.Add(dr);
                    return myResponse;
                }
            });
        ...
        /// <returns>a Dataset with a structure ready to be converted in JSON with Details about the test result</returns>
        public Task<DataSet> AddressCountVsMaxHist(DateTime startDate, DateTime endDate)
        {
            Int64 maxHist =1000;
            ...
            return Task.Run(() =>
            {
                try
                {
                    ...
                    bool stateTest = (dwhCount <= maxHist) && (dwhCount > 0) && (maxHist > 0);

                    testInterpretation = results.createMessageNotification(TestResult.BusinessStar.BU, TestResult.Entity.DimAddress, TestResult.TestGenericName.DistinctVsHistoric, maxHist, dwhCount, appEnv, !stateTest);
                ...
                {  
                    DataRow dr = myResponse.Tables[0].NewRow();
                    dr["description"] = ("Error Reading Max Hist ADDR Count from CCB: " + e.ToString().Substring(0, 198));
                    myResponse.Tables[0].Rows.Add(dr);
                    return myResponse;
                }
            });
        ...
        /// <returns>a Dataset with a structure ready to be converted in JSON with Details about the test result</returns>
        public Task<DataSet> StatisticalAddressEvaluation(DateTime evalDate, Int32 DAY_RANGE_TO_BE_EVALUATED, Double TOLERANCE_PERCENTAGE_IN_AVERAGE_VARIATION)
        {
            ...
            return Task.Run(() =>
            {
                try
                {
                    ...
                    testInterpretation = results.createMessageNotification(TestResult.BusinessStar.BU, TestResult.Entity.DimAddress, TestResult.TestGenericName.Statistical, ccbCount, Convert.ToInt64(Math.Round(averCount)), appEnv, stateTest);
                ...
                {                    
                    DataRow dr = myResponse.Tables[0].NewRow();
                    dr["description"] = ("Error Reading Statistical ADDR Count from CCB: " + e.ToString().Substring(0, 198));
                    myResponse.Tables[0].Rows.Add(dr);
                    return myResponse;
                }
            });
