        ...
        public Task<DataSet> DistinctAccountCounts(DateTime startDate, DateTime endDate)
        {
            ...
            return Task.Run(() =>
            {
                try
                {
                    ...
                    string interpoledQueryDTW = "SELECT COUNT(DISTINCT SRC_ACCT_ID) DTW_Count from [dwadm2].[CD_ACCT] where DATA_LOAD_DTTM BETWEEN '" + startDate.ToString("yyyy-MM-dd HH:mm") + "' AND '" + endDate.ToString("yyyy-MM-dd HH:mm") + "'";
        ...
        public Task<DataSet> NewAccountCounts(DateTime startDate, DateTime endDate)
        {
            ...
            return Task.Run(() =>
            {
                ...
                {             
                    ...
                    int cdcCount = evalDataCDC.Tables[0].Select("toInsert=1 AND ([__$operation]=1 OR [__$operation]=2)").Length;
                    int dtwCount = Convert.ToInt32(evalDataDTW.Tables[0].Rows[0][0]);
