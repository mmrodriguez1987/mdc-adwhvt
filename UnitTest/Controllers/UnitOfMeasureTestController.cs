...
namespace UnitTest.Controllers
{
    ...
    {    
        ...
        public async Task<IActionResult> Get(DateTime startDate, DateTime endDate, Boolean sendSMSNotify, Boolean saveResult, Int32 startHour, Int32 endHour)
        {
            ...
            dsResult = await uom.UOMCount(startDate, endDate);
           ...
           if (dsResult.Tables[0].Rows[0][2].ToString().StartsWith("Error"))
            ...
            dsResult = await uom.NewUOMCount(startDate,endDate);
            finalResultDS.Tables[0].ImportRow(dsResult.Tables[0].Rows[0]);
           ...
           if (dsResult.Tables[0].Rows[0][2].ToString().StartsWith("Error"))
            ...
            dsResult = await uom.UpdatedUOMCount(startDate, endDate);
            finalResultDS.Tables[0].ImportRow(dsResult.Tables[0].Rows[0]);
           ...
           if (dsResult.Tables[0].Rows[0][2].ToString().StartsWith("Error"))
            ...
            dsResult = await uom.StatisticalUOMEvaluation(endDate, gbl.EvaluatedDatesRangeOnAverageTest, gbl.ToleranceVariatonNumber);
            finalResultDS.Tables[0].ImportRow(dsResult.Tables[0].Rows[0]);
           ...
           if (dsResult.Tables[0].Rows[0][2].ToString().StartsWith("Error"))
            ...
            dsResult = await uom.UOMCountVsMaxHistoric(startDate, endDate);
            finalResultDS.Tables[0].ImportRow(dsResult.Tables[0].Rows[0]);
