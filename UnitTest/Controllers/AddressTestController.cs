...
namespace UnitTest.Controllers
{
    ...
    public class AddressTestController : ControllerBase
    {
        ...
        public async Task<IActionResult> Get(DateTime startDate, DateTime endDate, Boolean sendSMSNotify, Boolean saveResult, Int32 startHour, Int32 endHour)
        {
            ...
            #region Validation: Address Count
            dsResult = await addr.AddressCount(startDate, endDate);
           ...
           if (dsResult.Tables[0].Rows[0][2].ToString().StartsWith("Error"))
            ...
            #region Validation: New Address Count
            dsResult = await addr.NewAddressCount(startDate, endDate);
            finalResultDS.Tables[0].ImportRow(dsResult.Tables[0].Rows[0]);
           ...
           if (dsResult.Tables[0].Rows[0][2].ToString().StartsWith("Error"))
            ...
            #region Validation: Updated Address Count
            dsResult = await addr.UpdatedAddressCount(startDate, endDate);
            finalResultDS.Tables[0].ImportRow(dsResult.Tables[0].Rows[0]);
           ...
           if (dsResult.Tables[0].Rows[0][2].ToString().StartsWith("Error"))
            ...
            #region Validation: Statistical comparision of Address
            dsResult = await addr.StatisticalAddressEvaluation(endDate, gbl.EvaluatedDatesRangeOnAverageTest, gbl.ToleranceVariatonNumber);
            finalResultDS.Tables[0].ImportRow(dsResult.Tables[0].Rows[0]);
           ...
           if (dsResult.Tables[0].Rows[0][2].ToString().StartsWith("Error"))
            ...
            #endregion
            /*
            #region Validation: Address Count vs Max Historic
            dsResult = await addr.AddressCountVsMaxHist(startDate, endDate);
            finalResultDS.Tables[0].ImportRow(dsResult.Tables[0].Rows[0]);
