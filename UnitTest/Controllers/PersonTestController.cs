...
namespace UnitTest.Controllers
{
    ...
    {    
        ...
        public async Task<IActionResult> Get(DateTime startDate, DateTime endDate, Boolean sendSMSNotify, Boolean saveResult, Int32 startHour, Int32 endHour)
        {
            ...
            dsResult = await per.PersonCount(startDate, endDate);
           ...
           if (dsResult.Tables[0].Rows[0][2].ToString().StartsWith("Error"))
            ...
            dsResult = await per.NewPersonCount(startDate,endDate);
            finalResultDS.Tables[0].ImportRow(dsResult.Tables[0].Rows[0]);
           ...
           if (dsResult.Tables[0].Rows[0][2].ToString().StartsWith("Error"))
            ...
            dsResult = await per.UpdatedPersonCount(startDate, endDate);
            finalResultDS.Tables[0].ImportRow(dsResult.Tables[0].Rows[0]);            
           ...
           if (dsResult.Tables[0].Rows[0][2].ToString().StartsWith("Error"))
            ...
            dsResult = await per.StatisticalPersonEvaluation(endDate, gbl.EvaluatedDatesRangeOnAverageTest, gbl.ToleranceVariatonNumber);
            finalResultDS.Tables[0].ImportRow(dsResult.Tables[0].Rows[0]);
           ...
           /*
            ...
            dsResult = await per.PersonCountVsMaxHistoric(startDate, endDate);
            finalResultDS.Tables[0].ImportRow(dsResult.Tables[0].Rows[0]);
